========================================================================
  БИБЛИОТЕКА АКТИВНЫХ ШАБЛОНОВ. Обзор проекта RadarMap
========================================================================

Этот проект RadarMap создан автоматически с помощью мастера 
...
RadarMap.idl
    ...
    компонентов:
        Определения интерфейса C++ и 
            объявления идентификаторов GUID     (RadarMap.h).
        Определения идентификаторов GUID        (RadarMap_i.c).
        Библиотека типов                        (RadarMap.tlb).
        Код маршалинга                          (RadarMap_p.c и 
                                                   dlldata.c).

RadarMap.h
    В этом файле содержатся определения интерфейса C++ и объявления 
    идентификаторов GUID для элементов, определенных в 
    RadarMap.idl. В процессе компиляции файл повторно создается 
    компилятором MIDL.
...
StdAfx.h, StdAfx.cpp
    ...
    (PCH) с именем RadarMap.pch и файла предкомпилированных типов 
    с именем StdAfx.obj.
